...
const OrdersModify = () => {
    ...
    useEffect(async () => {
        ...
        const dataFetch = await axios
            .get(`/api/admin/orders/${id}?key=${key}`)
    ...
    return (
        <div className="w-full p-10 bg-white shadow">
            <div className="col-lg-8">
                <h2 className="mb-3 text-xl">Order ID : {data && data?.order_id}</h2>

                {data &&
                    <>
                        <h2 className="mb-3 text-xl">Payment Status :
                            <span className="font-bold">
                                {data && data?.payment?.status == 'settlement' ? ' Paid' : null}
                                {data && data?.payment?.status !== null ? null : ' Not Paid'}
                            </span>
                        </h2>
                ...
                <div className="block overflow-x-auto mb-8 lg:mb-0">
                    <table className="w-full text-left rounded-lg">
                        ...
                        <tbody>
                            {data ? data?.order_details?.map((item, i) =>
                        ...
                        {data &&
                            <tfoot className="w-full text-gray-800 bg-gray-50 whitespace-no-wrap">
                                ...
                                <tr className="border border-b-0">
                                    ...
                                    <td className="px-4 py-2">{NumberFormat(data?.shipping_cost, 'Rp.')}</td>
                                </tr>
                                <tr className="border border-b-0">
                                    <td className="px-4 py-2 text-right font-bold" colSpan="3">Discount</td>
                                    <td className="px-4 py-2">{NumberFormat(data?.couponamount ? -1 * data?.couponamount : 0, 'Rp.')}</td>
                                </tr>
                                <tr className="border">
                                    <td className="px-4 py-2 text-right font-bold" colSpan="3">Total</td>
                                    <td className="px-4 py-2">{NumberFormat(parseInt(data.net_price) + parseInt(data.shipping_cost) - parseInt(data?.couponamount ? data?.couponamount : 0), 'Rp.')}</td>
                                </tr>
                            </tfoot>
                        }
                    </table>
